...
public final class DetectionMagicEntrypoint implements IMagicAddonEntrypoint {
    @Override
    ...
    public void registerMagics(IMagicRegistry registry) {
        boolean registered = AddonMagicRegistration.register(
                registry,
                DetectionMagic.MAGIC_ID,
                DetectionMagic::execute,
                providerId(),
                MagicDisplayMetadata.CATEGORY_SPECIAL,
                "magic.typemoonworld.detection.name"
        );
        if (!registered) {
            TypeMoonAddon.LOGGER.error("Magic id '{}' is already registered; Detection was not installed",
                    DetectionMagic.MAGIC_ID);
        }
    }
}
