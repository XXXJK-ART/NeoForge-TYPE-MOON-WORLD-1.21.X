...
public final class AntoresMagicEntrypoint implements IMagicAddonEntrypoint {
    @Override
    ...
    public void registerMagics(IMagicRegistry registry) {
        boolean registered = AddonMagicRegistration.register(
                registry,
                AntoresMagic.MAGIC_ID,
                AntoresMagic::execute,
                providerId(),
                MagicDisplayMetadata.CATEGORY_SPECIAL,
                "magic.typemoonworld.antores.name"
        );
        if (!registered) {
            TypeMoonAddon.LOGGER.error("Magic id '{}' is already registered; Antores was not installed",
                    AntoresMagic.MAGIC_ID);
        }
    }
}
