...
public final class AndrephiusClientInput {
    private static boolean castKeyDown;
    private static boolean castOwned;

    private AndrephiusClientInput() {
    }
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        if (!castKeyDown && rawCastDown) {
            castOwned = canCharge;
            if (castOwned) {
                PacketDistributor.sendToServer(new AndrephiusCastInputPayload(
                        AndrephiusCastInputPayload.PRESS));
            }
        } else if (castKeyDown && !rawCastDown) {
            if (castOwned) {
                PacketDistributor.sendToServer(new AndrephiusCastInputPayload(
                        AndrephiusCastInputPayload.RELEASE));
            }
            castOwned = false;
        } else if (rawCastDown && castOwned && !canCharge) {
            PacketDistributor.sendToServer(new AndrephiusCastInputPayload(
                    AndrephiusCastInputPayload.CANCEL));
            castOwned = false;
        }
