...
import com.example.typemoonaddon.network.EffectPayload;
...
import com.example.typemoonaddon.magic.SakuraParticleService;
import com.example.typemoonaddon.shadowlogic.client.renderer.ShadowRibbonPlaneGeometry;
import com.example.typemoonaddon.shadowlogic.client.renderer.ShadowRibbonRenderStyle;
import com.example.typemoonaddon.shadowlogic.network.ShadowBindingEffectPayload;
import com.example.typemoonaddon.network.MagicOutputShockwavePayload;
...
public final class ClientEffects {
        ...
        while (iterator.hasNext()) {
            ...
            for (int i = 0; i < particles; i++) {
                ...
                minecraft.level.addParticle(
                    SakuraParticleService.particle(
                        effect.palette(),
    ...
    private static void spawnContinuousLink(Minecraft minecraft, Vec3 from, Vec3 to, byte palette) {
        ...
        for (int index = 0; index <= segments; index++) {
            ...
            ParticleOptions bodyParticle = palette == SakuraParticleService.SAKURA
                ? SakuraParticleService.SAKURA_DUST
            ...
            if (index % 4 == 0 && palette != SakuraParticleService.BLACK) {
                ...
                minecraft.level.addAlwaysVisibleParticle(
                    palette == SakuraParticleService.SAKURA
                        ? SakuraParticleService.SAKURA_DUST
    ...
    private static void tickShadowBindings(Minecraft minecraft) {
        ...
        while (bindings.hasNext()) {
            ...
            if (spawnMotes) {
                ringTargets.merge(target.getId(), binding.palette, SakuraParticleService::highest);
            }
    ...
    private static void spawnShadowBindingMote(Minecraft minecraft, Entity target, byte palette) {
        ...
        for (int mote = 0; mote < 2; mote++) {
            minecraft.level.addAlwaysVisibleParticle(
                ...
                0.0D,
                palette == SakuraParticleService.BLACK ? 0.006D : 0.012D,
                0.0D
            );
        }
    }
        ...
        for (Map.Entry<VoidLinkKey, ActiveShadowBinding> entry : SHADOW_BINDINGS.entrySet()) {
            if (entry.getKey().targetEntityId() == targetEntityId && entry.getValue().remainingTicks >= 0) {
                palette = palette == null
                    ...
                    : SakuraParticleService.highest(palette, entry.getValue().palette);
            }
        }
        return palette;
    }
