...
public class StoneManRenderer extends GeoEntityRenderer<StoneManEntity> {
    private static final ResourceLocation FALLBACK_BODY_TEXTURE =
            ResourceLocation.fromNamespaceAndPath("minecraft", "textures/block/stone.png");
    private static final Map<TextureVariantKey, ResourceLocation> TILED_TEXTURE_CACHE = new HashMap<>();
    ...
    private static final float TOP_LIGHT_BOOST = 1.10F;
    private static final float BOTTOM_DARKEN = 0.20F;
    ...
    private static final float RIDGE_LIGHT_STRENGTH = 0.80F;
    private static final float MICRO_CONTRAST_STRENGTH = 0.42F;
    private static final float CAVITY_MAX_DARKEN = 0.20F;
    private static final float RIDGE_MAX_LIGHTEN = 0.14F;
    private static final float MICRO_EDGE_MAX_LIGHTEN = 0.10F;
    ...
    private static final float CHEST_HALF_WIDTH = 0.20F;
    ...
    public StoneManRenderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new StoneManModel());
        this.withScale(1.2F);
        this.addRenderLayer(new StoneBodyTextureLayer(this));
        this.addRenderLayer(new TntFuseFlashLayer(this));
    }
    ...
    private static class StoneBodyTextureLayer extends GeoRenderLayer<StoneManEntity> {
        ...
        @Override
        public void render(PoseStack poseStack, StoneManEntity animatable, BakedGeoModel bakedModel, @Nullable RenderType renderType,
                           MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay) {
            BodyRenderData renderData = resolveBodyRenderData(animatable);
            ...
            Optional<GeoBone> faceBoneOptional = getGeoModel().getBone("face");
    ...
    private static class TntFuseFlashLayer extends GeoRenderLayer<StoneManEntity> {
        ...
        public void render(PoseStack poseStack, StoneManEntity animatable, BakedGeoModel bakedModel, @Nullable RenderType renderType,
            ...
            int flashColor = ((Mth.clamp((int) (alpha * 255.0F), 0, 255)) << 24) | 0x00FFFFFF;
    ...
    private static BodyRenderData resolveBodyRenderData(StoneManEntity animatable) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft == null || minecraft.getBlockRenderer() == null) {
            return new BodyRenderData(FALLBACK_BODY_TEXTURE, 0xFFFFFFFF);
        }

        BlockState mimicState = animatable.getMimicBlockState();
        TextureSet textureSet = resolveTextureSet(minecraft, mimicState);
        ...
        return new BodyRenderData(texture, tint);
    }

    private static TextureSet resolveTextureSet(Minecraft minecraft, BlockState state) {
        TextureAtlasSprite particleSprite = minecraft.getBlockRenderer()
                .getBlockModelShaper()
                .getParticleIcon(state);
        ResourceLocation particleTexture = particleSprite == null
                ? FALLBACK_BODY_TEXTURE
                : toTexturePath(particleSprite.contents().name());
        ...
        ResourceLocation sideFallback = firstAvailableTexture(
                minecraft,
                northRaw, southRaw, eastRaw, westRaw,
                particleTexture,
                FALLBACK_BODY_TEXTURE
        );
        ResourceLocation up = firstAvailableTexture(
                minecraft,
                upRaw,
                sideFallback,
                particleTexture,
        ...
        ResourceLocation down = firstAvailableTexture(
                minecraft,
                downRaw,
                sideFallback,
                particleTexture,
        ...
        return new TextureSet(up, down, north, south, west, east);
    }
    ...
    private static @Nullable ResourceLocation findDirectionTexture(Minecraft minecraft, BakedModel model, BlockState state, Direction direction) {
        ...
        List<BakedQuad> generalQuads = model.getQuads(state, null, RandomSource.create(42L));
    ...
    private static @Nullable ResourceLocation firstTextureFromQuads(List<BakedQuad> quads) {
        for (BakedQuad quad : quads) {
    ...
    private static ResourceLocation getOrCreateTiledTexture(TextureSet textureSet, int variant) {
        TextureVariantKey key = new TextureVariantKey(textureSet, variant);
        ...
        try {
            ...
            int tileWidth = Math.max(
                    ...
                    Math.max(
                            ...
                            Math.max(
                                    Math.max(northImage.getWidth(), southImage.getWidth()),
                                    Math.max(westImage.getWidth(), eastImage.getWidth())
                            )
                    )
            );
            int tileHeight = Math.max(
                    ...
                    Math.max(
                            ...
                            Math.max(
                                    Math.max(northImage.getHeight(), southImage.getHeight()),
                                    Math.max(westImage.getHeight(), eastImage.getHeight())
                            )
                    )
            );
            ...
            for (int y = 0; y < outHeight; y++) {
                int yPercent = (int) ((long) y * 100L / Math.max(1, outHeight - 1));
                for (int x = 0; x < outWidth; x++) {
                    int cellX = x / tileWidth;
                    int cellY = y / tileHeight;
                    NativeImage source = pickRandomFaceImage(
                            upImage, downImage, northImage, southImage, westImage, eastImage,
                            cellX, cellY, yPercent, variant
                    );
    ...
    private static @Nullable NativeImage readTextureImage(Minecraft minecraft, ResourceLocation texture) throws IOException {
    ...
    private static @Nullable NativeImage readAndTrackImage(Minecraft minecraft, ResourceLocation texture, Set<NativeImage> collector) throws IOException {
    ...
    private static @Nullable NativeImage firstNonNullImage(@Nullable NativeImage... images) {
    ...
    private static NativeImage pickRandomFaceImage(
        ...
        if (yPercent >= (100 - BOTTOM_BAND_PERCENT) && selector < BOTTOM_FACE_BIAS_PERCENT) {
            return downImage;
        }
    ...
    private static int hashCell(int x, int y, int variant) {
        int h = x * 0x1f1f1f1f ^ y * 0x45d9f3b ^ (variant * 0x9e3779b9);
        h ^= (h >>> 16);
        h *= 0x27d4eb2d;
        h ^= (h >>> 15);
        return h;
    ...
    private static int applyPseudoShading(int argb, int x, int y, int width, int height) {
        ...
        float u = width <= 1 ? 0.5F : (float) x / (float) (width - 1);
        float v = height <= 1 ? 0.5F : (float) y / (float) (height - 1);
        ...
        float lightFactor = clamp(verticalFactor * edgeFactor, 0.70F, 1.20F);
    ...
    private static int applyBodyRegionMaterialProfile(int argb, int x, int y, int width, int height, int variant) {
        ...
        float macroNoise = (hashToUnit(hashCell(x / 6, y / 6, variant ^ 0x5A5A5A5A)) * 2.0F) - 1.0F;
        float microNoise = (hashToUnit(hashCell(x, y, variant ^ 0x27D4EB2D)) * 2.0F) - 1.0F;
    ...
    private record TextureSet(
        ...
        private String cacheId() {
            return up.toDebugFileName()
                    + "_" + down.toDebugFileName()
                    + "_" + north.toDebugFileName()
                    + "_" + south.toDebugFileName()
                    + "_" + west.toDebugFileName()
                    + "_" + east.toDebugFileName();
        }
    }

    private record TextureVariantKey(TextureSet set, int variant) {
    }
