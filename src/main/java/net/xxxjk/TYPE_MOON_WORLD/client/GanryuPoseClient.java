...
public final class GanryuPoseClient {
   private static final Map<UUID, PoseState> POSES = new ConcurrentHashMap<>();

   private GanryuPoseClient() {}

   public static void apply(UUID playerId, GanryuMove move, int ticks) {
      if (playerId == null) return;
      if (ticks <= 0) {
         POSES.remove(playerId);
      } else if (move != null) {
         POSES.put(playerId, new PoseState(move, gameTime() + ticks));
      }
   }

   public static GanryuMove getMove(Player player) {
   ...
   private record PoseState(GanryuMove move, long until) {}
}
