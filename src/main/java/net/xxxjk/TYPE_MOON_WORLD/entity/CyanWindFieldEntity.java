...
public class CyanWindFieldEntity extends Entity {
   ...
   public CyanWindFieldEntity(Level level, double x, double y, double z, float radius, int duration, LivingEntity owner) {
      this((EntityType<?>)ModEntities.CYAN_WIND_FIELD.get(), level);
      this.setPos(x, y, z);
   ...
   public boolean isTornadoMode() {
      return (Boolean)this.entityData.get(TORNADO_MODE);
   }
      ...
      if (this.level().isClientSide) {
         ...
         if (tornado) {
            ...
            for (int arm = 0; arm < arms; arm++) {
               double armPhase = (Math.PI * 2) * arm / arms;
            ...
            for (int ix = 0; ix < wisps; ix++) {
               double t = this.random.nextDouble();
               double baseRadius = 0.25 + 0.5 * t + 0.2 * t * t;
               double wobble = 0.07 * Math.sin(t * 5.0 * Math.PI + this.tickCount * 0.12 + ix);
               double radius = currentRadius * Math.max(0.1, baseRadius + wobble);
            ...
            for (int ix = 0; ix < 5; ix++) {
               ...
               this.level().addParticle(ParticleTypes.CLOUD, d0, d1, d2, 0.0, 0.2 + this.random.nextDouble() * 0.1, 0.0);
               if (this.random.nextFloat() < 0.3F) {
                  this.level().addParticle(ParticleTypes.SWEEP_ATTACK, d0, d1, d2, 0.0, 0.1, 0.0);
               }

               if (this.random.nextFloat() < 0.1F) {
                  this.level().addParticle(ParticleTypes.POOF, d0, d1, d2, 0.0, 0.3, 0.0);
               }
            }
         }
      } else {
         ...
         List<LivingEntity> list = this.level()
            .getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(currentRadius, currentRadius, currentRadius));
         LivingEntity ownerEntity = this.getOwner();
         boolean tornado = this.isTornadoMode();

         for (LivingEntity entity : list) {
            if (!EntityUtils.isImmunePlayerTarget(entity)) {
               ...
               if (!(dx * dx + dz * dz > currentRadius * currentRadius)) {
                  ...
                  if (entity != ownerEntity) {
                     if (tornado) {
                        ...
                        Vec3 dir;
                        if (distSqr < 0.001) {
                           dir = new Vec3(this.random.nextDouble() - 0.5, 0.0, this.random.nextDouble() - 0.5).normalize();
                        } else {
                           double dist = Math.sqrt(distSqr);
                           dir = new Vec3(-dx / dist, 0.0, -dz / dist);
                        }

                        Vec3 tangent = new Vec3(-dir.z, 0.0, dir.x);
         ...
         for (Projectile projectile : this.level()
            .getEntitiesOfClass(Projectile.class, this.getBoundingBox().inflate(currentRadius, currentRadius, currentRadius))) {
            if (!projectile.isRemoved() && (ownerEntity == null || projectile.getOwner() != ownerEntity)) {
