...
public class MagicCopyingTableMenu extends AbstractContainerMenu {
   ...
   public void tryCopy(ServerPlayer player,String id){var vars=player.getData(TypeMoonWorldModVariables.PLAYER_VARIABLES);if(!MagicLearningStrategy.isLearned(vars,id)||MagicProficiencyService.get(vars,id)<100||!MagicLearningStrategy.canCopy(id)||!MagicLearningStrategy.materialAllowed(vars,id)){player.displayClientMessage(Component.translatable("message.typemoonworld.magic.copy_locked"),true);return;}ItemStack paper=items.getStackInSlot(0),ink=items.getStackInSlot(1);if(!paper.is(Items.PAPER)||!ink.is(Items.INK_SAC)){player.displayClientMessage(Component.translatable("message.typemoonworld.magic.copy_need_material"),true);return;}var pageItem=net.minecraft.core.registries.BuiltInRegistries.ITEM.get(net.minecraft.resources.ResourceLocation.fromNamespaceAndPath("typemoonworld",MagicLearningStrategy.pageItemPath(id)));if(pageItem==Items.AIR){player.displayClientMessage(Component.translatable("message.typemoonworld.magic.copy_locked"),true);return;}paper.shrink(1);ink.shrink(1);ItemStack page=new ItemStack(pageItem);if(!player.addItem(page))player.drop(page,false);blockEntity.setChanged();broadcastChanges();}
}
