...
public final class RuneProgramCostService {
   ...
   public static double calculate(RuneProgram program) {
      if (program == null) return 0.0D;
      int triggers = 0, modifiers = 0; for (String id : program.slots(RunePosition.TRIGGER)) if (!id.isEmpty()) triggers++; for (String id : program.slots(RunePosition.MODIFIER)) if (!id.isEmpty()) modifiers++;
      return baseCost(program) * (1.0D + triggers * 0.3D + modifiers * 0.2D);
   }
}
