...
public class MagicEmeraldWinterRiver {
   public static void execute(Entity entity) {
      if (entity instanceof Player player) {
         ...
         if (count < 2) {
            player.displayClientMessage(Component.translatable("message.typemoonworld.magic.emerald_winter_river.need_gem"), true);
         } else {
   ...
   private static void activateWinterRiver(Player player, float multiplier) {
      ...
      if (!level.isClientSide) {
         ...
         if (level instanceof ServerLevel serverLevel) {
            serverLevel.sendParticles(ParticleTypes.HAPPY_VILLAGER, center.getX(), center.getY() + 1, center.getZ(), 50, 2.0, 2.0, 2.0, 0.1);
         }
         ...
         for (Entity e : level.getEntities(player, box, ex -> ex != player)) {
            ...
            dir = dir.normalize();
            Vec3 target = cPos.add(dir.scale(radius * 1.5 + 2.0));
            ...
            for (int y = 0; y <= height; y++) {
               for (int z = -radius; z <= radius; z++) {
                  ...
                  if (isBorder) {
                     ...
                     if (currentState.is((Block)ModBlocks.GREEN_TRANSPARENT_BLOCK.get())) {
                        int delay = 160 + random.nextInt(41);
                        level.scheduleTick(pos, (Block)ModBlocks.GREEN_TRANSPARENT_BLOCK.get(), delay);
                     } else if (currentState.getDestroySpeed(level, pos) >= 0.0F && !currentState.is(Blocks.BEDROCK)) {
                        replaceBlockWithoutDrops(level, pos, ((Block)ModBlocks.GREEN_TRANSPARENT_BLOCK.get()).defaultBlockState());
                     }
                  } else {
         ...
         if (airborneCast) {
            ...
            double targetY = lockPos.y - player.getBbHeight() * 0.5;
