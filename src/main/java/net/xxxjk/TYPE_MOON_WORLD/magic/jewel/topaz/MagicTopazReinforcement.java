...
public class MagicTopazReinforcement {
   public static void execute(Entity entity) {
      if (entity instanceof Player player) {
         ...
         if (count < 3) {
            player.displayClientMessage(Component.translatable("message.typemoonworld.magic.topaz_reinforcement.need_gem"), true);
         } else {
   ...
   private static void applyReinforcement(Player player, float multiplier) {
      ...
      if (player instanceof LivingEntity) {
         ...
         if (player.level() instanceof ServerLevel serverLevel) {
            serverLevel.sendParticles(ParticleTypes.ELECTRIC_SPARK, player.getX(), player.getY() + 1.0, player.getZ(), 30, 0.5, 1.0, 0.5, 0.1);
            serverLevel.sendParticles(ParticleTypes.WAX_ON, player.getX(), player.getY() + 1.0, player.getZ(), 20, 0.5, 1.0, 0.5, 0.1);
         }
      }
   }
}
