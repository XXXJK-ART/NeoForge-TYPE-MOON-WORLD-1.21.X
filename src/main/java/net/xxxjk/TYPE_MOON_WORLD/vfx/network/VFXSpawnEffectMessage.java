...
public record VFXSpawnEffectMessage(String effectId, double x, double y, double z, Optional<UUID> targetEntityUuid, String dimension, long seed)
   ...
   private static VFXSpawnEffectMessage read(FriendlyByteBuf buffer) {
      ...
      return new VFXSpawnEffectMessage(effectId, x, y, z, target, dimension, seed);
   }
      ...
      if (context.flow() == PacketFlow.CLIENTBOUND) {
         context.enqueueWork(() -> {
            if (FMLEnvironment.dist == Dist.CLIENT) {
               VFXClientRuntime.spawn(message.effectId, message.x, message.y, message.z, message.targetEntityUuid, message.seed);
            }
         });
      }
   }
