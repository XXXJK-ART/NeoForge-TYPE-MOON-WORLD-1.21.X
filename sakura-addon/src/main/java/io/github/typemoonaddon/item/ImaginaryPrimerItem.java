...
public final class ImaginaryPrimerItem extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (data.learned()) {
            if (!TypeMoonIntegration.ensureImaginaryAttribute(serverPlayer)) {
                serverPlayer.displayClientMessage(Component.translatable("message.typemoonaddon.attribute_failed"), false);
                return InteractionResultHolder.fail(held);
            }
            serverPlayer.displayClientMessage(Component.translatable("message.typemoonaddon.already_learned"), true);
            return InteractionResultHolder.fail(held);
        }

        if (!TypeMoonIntegration.learnAndGrantAttribute(serverPlayer)) {
            serverPlayer.displayClientMessage(Component.translatable("message.typemoonaddon.attribute_failed"), false);
            return InteractionResultHolder.fail(held);
        }
        data.unlock();
        ...
        serverPlayer.displayClientMessage(Component.translatable("message.typemoonaddon.learned"), false);
