    ...
    public static InteractionResultHolder<ItemStack> useForRitual(Level level, Player player, InteractionHand usedHand) {
        ...
        if (!isRitualPair(used, other)) {
            serverPlayer.displayClientMessage(Component.translatable(
                "message.typemoonaddon.holy_grail.requires_pair"
            ), true);
            return InteractionResultHolder.fail(used);
        }
        ...
        if (!data.learned()) {
            serverPlayer.displayClientMessage(Component.translatable(
                "message.typemoonaddon.holy_grail.requires_imaginary"
            ), true);
            return InteractionResultHolder.fail(used);
        }
        if (data.grailWormAscended()) {
            TypeMoonIntegration.ensureGrailWormPower(serverPlayer);
            serverPlayer.displayClientMessage(Component.translatable(
                "message.typemoonaddon.holy_grail.already_used"
            ), true);
            return InteractionResultHolder.fail(used);
        }
        RuleBreakerDispelService.capturePreGrailState(serverPlayer);
        if (!TypeMoonIntegration.performHolyGrailRitual(serverPlayer)
            || !TypeMoonIntegration.isBlackMudControlLearned(serverPlayer)
            || !TypeMoonIntegration.isSummonBlackMudLearned(serverPlayer)
            || !TypeMoonIntegration.isShadowBindingLearned(serverPlayer)) {
            if (!data.grailWormAscended()) {
                data.clearGrailPowerSnapshot();
            }
            serverPlayer.displayClientMessage(Component.translatable(
                "message.typemoonaddon.holy_grail.failed"
            ), false);
            return InteractionResultHolder.fail(used);
        }
        RuleBreakerDispelService.lockGrailMagicKnowledge(serverPlayer);

        ImaginaryStorageService.cancelChannelsForUpgrade(serverPlayer);
        ImaginaryShadowService.stopForUpgrade(serverPlayer);
        BlackMudService.clearMovementRestriction(serverPlayer);
        ...
        serverPlayer.displayClientMessage(Component.translatable(
            "message.typemoonaddon.holy_grail.awakened"
        ), false);
        return InteractionResultHolder.success(used);
    }

    private static boolean isRitualPair(ItemStack first, ItemStack second) {
        return first.is(ModItems.HOLY_GRAIL_FRAGMENT.get()) && second.is(ModItems.CREST_WORM.get())
            || first.is(ModItems.CREST_WORM.get()) && second.is(ModItems.HOLY_GRAIL_FRAGMENT.get());
    }
}
