...
public final class PlayerArmorOcclusion {
    ...
    private static OcclusionMask mask(Player player) {
        CursedArmorState cursedState = player.getData(ModAttachments.CURSED_ARMOR_VIEW.get()).state();
        if (cursedState == CursedArmorState.ACTIVE) {
            return OcclusionMask.CURSED_ARMOR;
        }
        if (player.getItemBySlot(EquipmentSlot.CHEST).is(ModItems.VOID_RING_REGALIA.get())) {
