...
import io.github.typemoonaddon.network.SelectShadowTransferPayload;
...
import io.github.typemoonaddon.client.AddonGuiUtils;

/** Tactical coordinate map for selecting an owned Shadow Familiar. */
public final class ShadowTransferScreen extends Screen {
    ...
    public ShadowTransferScreen(OpenShadowTransferPayload payload) {
        super(Component.translatable("screen.typemoonaddon.shadow_transfer.title"));
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        AddonGuiUtils.renderScreenBackdrop(gui, this.width, this.height);
        ...
        AddonGuiUtils.renderArcaneWindow(gui, panelX, panelY, MAP_PANEL_WIDTH, MAP_PANEL_HEIGHT, ACCENT);
        AddonGuiUtils.renderArcanePanel(gui, mapX, mapY, mapWidth, mapHeight, ACCENT);
        gui.drawCenteredString(this.font, this.title, this.width / 2, panelY + 9, AddonGuiUtils.ARCANE_TEXT);
        Component count = Component.translatable("screen.typemoonaddon.shadow_transfer.target_count", this.targets.size());
        gui.drawString(
            this.font,
            count,
            panelX + MAP_PANEL_WIDTH - 12 - this.font.width(count),
            panelY + 9,
            AddonGuiUtils.ARCANE_TEXT_MUTED,
            false
        );
        ...
        if (this.targets.isEmpty()) {
            gui.drawCenteredString(
                this.font,
                Component.translatable("screen.typemoonaddon.shadow_transfer.no_targets"),
                mapX + mapWidth / 2,
                mapY + mapHeight / 2 - 4,
                AddonGuiUtils.ARCANE_TEXT_MUTED
            );
        ...
        if (this.hoveredTarget != null) {
            gui.renderTooltip(this.font, List.of(
                Component.translatable(
                    "screen.typemoonaddon.shadow_transfer.coordinates",
                    this.hoveredTarget.x(),
                    this.hoveredTarget.y(),
                    this.hoveredTarget.z()
                ),
                Component.translatable(
                    "screen.typemoonaddon.shadow_transfer.cost",
                    this.hoveredTarget.distance(),
                    this.hoveredTarget.manaCost()
                )
    ...
    public void renderBackground(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        // Keep the world visible behind the tactical map.
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0 && !this.targets.isEmpty()) {
            int panelX = (this.width - MAP_PANEL_WIDTH) / 2;
            int panelY = (this.height - MAP_PANEL_HEIGHT) / 2;
            List<Marker> markers = layoutMarkers(
                panelX + 12,
                panelY + 36,
                MAP_PANEL_WIDTH - 24,
                MAP_PANEL_HEIGHT - 49
    ...
    private void renderMarkers(GuiGraphics gui, int mouseX, int mouseY, int mapX, int mapY, int mapWidth, int mapHeight) {
        ...
        for (int index = 0; index < markers.size(); index++) {
            ...
            int accent = hovered ? AddonGuiUtils.ARCANE_VALID : selected ? AddonGuiUtils.ARCANE_GOLD : ACCENT;
            gui.fill(marker.x, marker.y, marker.x + MARKER_SIZE, marker.y + MARKER_SIZE,
                hovered || selected ? 0xF02A171B : AddonGuiUtils.ARCANE_PANEL_ALT);
        ...
        gui.fill(origin[0] - 4, origin[1], origin[0] + 5, origin[1] + 1, AddonGuiUtils.ARCANE_TEXT);
        gui.fill(origin[0], origin[1] - 4, origin[0] + 1, origin[1] + 5, AddonGuiUtils.ARCANE_TEXT);
    }
