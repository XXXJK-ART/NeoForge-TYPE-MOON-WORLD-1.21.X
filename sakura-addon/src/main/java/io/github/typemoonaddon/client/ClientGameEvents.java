...
import io.github.typemoonaddon.network.OpenSpacePayload;
import io.github.typemoonaddon.network.BlockAbsorptionHoldPayload;
import io.github.typemoonaddon.network.ShadowMaterializationHoldPayload;
...
import io.github.typemoonaddon.client.renderer.PlayerArmorOcclusion;
...
public final class ClientGameEvents {
    ...
    public static void onRenderArm(RenderArmEvent event) {
        ...
        if (renderer instanceof PlayerRenderer playerRenderer) {
            ...
            if (event.getPlayer().getData(ModAttachments.CURSED_ARMOR_VIEW.get()).state() == CursedArmorState.ACTIVE
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        var imaginarySpace = minecraft.player.getData(ModAttachments.IMAGINARY_SPACE.get());
        boolean showErosion = imaginarySpace.grailWormAscended();
        LivingEntity shown = PollutionService.isPolluted(minecraft.player) ? minecraft.player : null;
        LivingEntity spiritualShown = minecraft.player.getData(ModAttachments.SPIRITUAL_DAMAGE.get()).percent() > 0 ? minecraft.player : null;
        if (shown == null && minecraft.hitResult instanceof EntityHitResult entityHit
            && entityHit.getEntity() instanceof LivingEntity living
            && PollutionService.isPolluted(living)) {
            shown = living;
        }
        if (minecraft.hitResult instanceof EntityHitResult entityHit
            && entityHit.getEntity() instanceof LivingEntity living
            && living.getData(ModAttachments.SPIRITUAL_DAMAGE.get()).percent() > 0) {
        ...
        if (shown != null) {
            float progress = PollutionService.progress(shown);
            renderProgressBar(
                event,
                minecraft,
                x,
        ...
        if (spiritualShown != null) {
            var spiritual = spiritualShown.getData(ModAttachments.SPIRITUAL_DAMAGE.get());
