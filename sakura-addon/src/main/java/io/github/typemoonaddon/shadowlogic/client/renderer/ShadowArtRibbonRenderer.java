...
public final class ShadowArtRibbonRenderer extends EntityRenderer<ShadowArtRibbonEntity> {
    ...
    public void render(
        ...
        byte palette = GrailParticleService.palette(owner);
    ...
    private static Curve actionCurve(ShadowArtRibbonEntity ribbon, Vec3 root, Vec3 tip, float partialTick) {
        int index = ribbon.ribbonIndex();
        ...
        return switch (ribbon.action()) {
            case ShadowArtRibbonEntity.ATTACK -> {
            ...
            case ShadowArtRibbonEntity.LIFT -> new Curve(
                root,
            ...
            case ShadowArtRibbonEntity.THROW -> {
            ...
            case ShadowArtRibbonEntity.PINNED, ShadowArtRibbonEntity.PIERCED -> {
                double side = index % 2 == 0 ? 1.0D : -1.0D;
            ...
            case ShadowArtRibbonEntity.DEFEND -> new Curve(
                root,
            ...
            case ShadowArtRibbonEntity.RETRACT -> new Curve(
                root,
    ...
    private static double sourceModelWidth(byte action, double progress) {
        ...
        if (action == ShadowArtRibbonEntity.ATTACK && progress > 0.82D) return base * (1.0D - (progress - 0.82D) * 2.1D);
    ...
    private static Entity owner(ShadowArtRibbonEntity ribbon) {
    ...
    @Override
    public ResourceLocation getTextureLocation(ShadowArtRibbonEntity entity) {
